...
const SpeakerCard = ({ name, company, imageSrc, topic, bioLink }) => {
  ...
  return (
    <div className="group">
      ...
      <div className="flex p-4 mt-12 lg:mt-16 justify-center w-full h-18 sm:h-36">
        <Link href={bioLink}>
          <div className="inline-flex items-center justify-between w-full h-full">
            ...
            <div className="flex-shrink-0">
              <ChevronDoubleRightIcon className="text-wri-red w-8 lg:w-16 h-full" />{" "}
            </div>
          </div>
        </Link>
      </div>
      <div className="flex justify-center mx-10 my-6 sm:my-2 mb-6 xl:mt-6">
        <Link href={bioLink}>
          <div className="rounded-md text-center text-xl text-white hover:text-wri-dark-blue hover:shadow-xl bg-wri-red py-6 px-12 cursor-pointer whitespace-nowrap">
            Bio & Abstract
          </div>
        </Link>
      </div>
